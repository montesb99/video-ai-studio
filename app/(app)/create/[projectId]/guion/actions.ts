...
export type GenerateScriptResult =
  ...
  | {
      ok: false;
      reason: "missing_proposal" | "generation_failed" | "no_workspace" | "not_configured" | "already_generating";
    };
...
export async function generateScript(projectId: string): Promise<GenerateScriptResult> {
  ...
  const outcome = await runGenerateScript(owned.supabase, owned.workspaceId, projectId);
